        ...
        rules: [
          {
            ...
            expr: |||
              max without (endpoint) (
                sum without (instance) (up{%(etcdSelector)s} == bool 0)
              or
                count without (To) (
                  sum without (instance) (rate(etcd_network_peer_sent_failures_total{%(etcdSelector)s}[120s])) > 0.01
                )
              )
              > 0
            ||| % $._config,
          ...
          },
          {
            ...
            expr: |||
              sum(up{%(etcdSelector)s} == bool 1) without (instance) < ((count(up{%(etcdSelector)s}) without (instance) + 1) / 2)
            ||| % $._config,
            'for': '3m',
            labels: {
              severity: 'critical',
          ...
          },
          {
            alert: 'etcdNoLeader',
            expr: |||
              etcd_server_has_leader{%(etcdSelector)s} == 0
            ||| % $._config,
            'for': '1m',
            labels: {
              severity: 'critical',
          ...
          },
          {
            ...
            expr: |||
              increase((max without (instance) (etcd_server_leader_changes_seen_total{%(etcdSelector)s}) or 0*absent(etcd_server_leader_changes_seen_total{%(etcdSelector)s}))[15m:1m]) >= 4
            ||| % $._config,
            'for': '5m',
            labels: {
              severity: 'warning',
          ...
          },
          {
            ...
            expr: |||
              100 * sum(rate(grpc_server_handled_total{%(etcdSelector)s, grpc_code=~"Unknown|FailedPrecondition|ResourceExhausted|Internal|Unavailable|DataLoss|DeadlineExceeded"}[5m])) without (grpc_type, grpc_code)
                /
              sum(rate(grpc_server_handled_total{%(etcdSelector)s}[5m])) without (grpc_type, grpc_code)
                > 1
            ||| % $._config,
            'for': '10m',
            labels: {
          ...
          },
          {
            ...
            expr: |||
              histogram_quantile(0.99, sum(rate(grpc_server_handling_seconds_bucket{%(etcdSelector)s, grpc_method!="Defragment", grpc_type="unary"}[5m])) without(grpc_type))
              > 0.15
            ||| % $._config,
            'for': '10m',
            labels: {
          ...
          },
          {
            ...
            expr: |||
              histogram_quantile(0.99, rate(etcd_network_peer_round_trip_time_seconds_bucket{%(etcdSelector)s}[5m]))
              > 0.15
            ||| % $._config,
            'for': '10m',
            labels: {
          ...
          },
          {
            ...
            expr: |||
              rate(etcd_server_proposals_failed_total{%(etcdSelector)s}[15m]) > 5
            ||| % $._config,
            'for': '15m',
            labels: {
              severity: 'warning',
          ...
          },
          {
            ...
            expr: |||
              histogram_quantile(0.99, rate(etcd_disk_wal_fsync_duration_seconds_bucket{%(etcdSelector)s}[5m]))
              > 0.5
            ||| % $._config,
            'for': '10m',
            labels: {
          ...
          },
          {
            ...
            expr: |||
              histogram_quantile(0.99, rate(etcd_disk_backend_commit_duration_seconds_bucket{%(etcdSelector)s}[5m]))
              > 0.25
            ||| % $._config,
            'for': '10m',
            labels: {
          ...
          },
          {
            ...
            expr: |||
              (last_over_time(etcd_mvcc_db_total_size_in_bytes{%(etcdSelector)s}[5m]) / last_over_time(etcd_server_quota_backend_bytes{%(etcdSelector)s}[5m]))*100 > 95
            ||| % $._config,
            'for': '10m',
            labels: {
              severity: 'critical',
          ...
          },
          {
            ...
            expr: |||
              predict_linear(etcd_mvcc_db_total_size_in_bytes{%(etcdSelector)s}[4h], 4*60*60) > etcd_server_quota_backend_bytes{%(etcdSelector)s}
            ||| % $._config,
            'for': '10m',
            labels: {
              severity: 'warning',
          ...
          },
          {
            ...
            expr: |||
              (last_over_time(etcd_mvcc_db_total_size_in_use_in_bytes{%(etcdSelector)s}[5m]) / last_over_time(etcd_mvcc_db_total_size_in_bytes{%(etcdSelector)s}[5m])) < 0.5 and etcd_mvcc_db_total_size_in_use_in_bytes{%(etcdSelector)s} > 104857600
            ||| % $._config,
            'for': '10m',
            labels: {
              severity: 'warning',
